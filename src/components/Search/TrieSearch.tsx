...
const TrieSearch = ({ searchedWord }: { searchedWord: string }) => {
  ...
  useEffect(() => {
    const initWorker = async () => {
      try {
        const w = new Worker(
          new URL("../../webWorkers/TrieWebWorker.js", import.meta.url),
        );
        setWorker(w);
        ...
        w.onmessage = (e) => {
          ...
          if (e.data.type === "ERROR") {
            setError(e.data.message || "Failed to load trie");
          }
          if (e.data.type === "RESULTS") {
            ...
            setTelemetry({
              ...
              searchTime: (t.latencyUs ?? 0) / 1000, // µs → ms
              throughput: t.throughput ?? 0,
              memoryFootprint: t.memoryFootprint ?? 0,
              algorithmDetails: t.complexity ?? t.algorithmDetails ?? "O(L) prefix",
            });
          }
        };
      } catch (error) {
      ...
      {isWordsLoaded ? (
        <>
          ...
          <div className="pane-telemetry">
            ...
            <div className="pane-telemetry-pill">
              <span className="pane-badge">O(L)</span>
            </div>
          </div>
        ...
        <div className="pane-loading">Loading trie...</div>
      )}
    </div>
  );
};
